...
def plot_mem_spk(mem, spk, thr_line=False, vline=False, title=False, ylim_max=1.25):
    # Generate Plots
    fig, ax = plt.subplots(2, figsize=(8, 6), sharex=False, gridspec_kw={"height_ratios": [1, 0.4]})

    # Plot membrane potential
    ax[0].plot(mem)
    ax[0].set_ylim([0, ylim_max])
    ax[0].set_ylabel("Membrane Potential (U_mem)")
    if title:
        ax[0].set_title(title)
    if thr_line:
